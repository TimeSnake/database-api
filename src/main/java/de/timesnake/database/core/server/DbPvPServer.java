...
public abstract class DbPvPServer extends DbTaskServer implements
    de.timesnake.database.util.server.DbPvPServer {
  ...
  public void setPvP(boolean oldPvP) {
    super.setWithKey(oldPvP, Column.Server.OLD_PVP,
        () -> Channel.getInstance().sendMessage(new ChannelServerMessage<>(this.getName(), MessageType.Server.OLD_PVP
            , oldPvP)));
  }
}
