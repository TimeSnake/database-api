...
public class LocationsTable extends Table {
    ...
    public de.timesnake.database.util.object.DbLocation getLocation(Integer number, String worldName) {
        ...
        return new de.timesnake.database.util.object.DbLocation(worldName, super.getFirst(Column.Location.X, numberEntry, worldEntry), super.getFirst(Column.Location.Y, numberEntry, worldEntry), super.getFirst(Column.Location.Z, numberEntry, worldEntry), super.getFirst(Column.Location.YAW, numberEntry, worldEntry), super.getFirst(Column.Location.PITCH, numberEntry, worldEntry));
    }
    ...
    public void addLocation(Integer number, de.timesnake.database.util.object.DbLocation location, TableEntry<?> primaryEntries) {
        if (number == null) {
            return;
        }
        super.addEntry(new PrimaryEntries(new TableEntry<>(number, Column.Location.NUMBER), new TableEntry<>(location.getWorldName(), Column.Location.WORLD), primaryEntries), new TableEntry<>(number, Column.Location.NUMBER), new TableEntry<>(location.getWorldName(), Column.Location.WORLD), new TableEntry<>(location.getX(), Column.Location.X), new TableEntry<>(location.getY(), Column.Location.Y), new TableEntry<>(location.getZ(), Column.Location.Z), new TableEntry<>(location.getYaw(), Column.Location.YAW), new TableEntry<>(location.getPitch(), Column.Location.PITCH));
    }
    ...
    public void deleteSpawn(Integer number, String worldName) {
        ...
        super.deleteEntry(new TableEntry<>(number, Column.Location.NUMBER), new TableEntry<>(worldName, Column.Location.WORLD));
    }
