...
public abstract class KeyedQueryTool extends QueryTool {

  protected final PrimaryKeyEntries keyEntries;
  private final List<Entry<?>> keyEntryList;

  protected KeyedQueryTool(DatabaseConnector databaseConnector, String tableName, boolean temporary, Entry<?>... keys) {
    super(databaseConnector, tableName, UpdatePolicy.DISCARD_IF_NOT_EXISTS, temporary);
    this.keyEntries = new PrimaryKeyEntries(keys);
    this.keyEntryList = Arrays.asList(keys);
  }

  private Entry<?>[] addPrimaryKeys(Entry<?>... entries) {
    List<Entry<?>> entriesList = new ArrayList<>(keyEntryList);
