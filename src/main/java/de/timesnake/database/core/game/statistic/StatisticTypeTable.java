...
public class StatisticTypeTable extends DefinitionAndQueryTool {

  protected StatisticTypeTable(DatabaseConnector databaseConnector, String tableName) {
    super(databaseConnector, tableName, Column.Game.GAME_NAME, Column.Game.STAT_TYPE_NAME);
  ...
  public Set<StatType<?>> getStats(String gameName) {
    ...
    for (ColumnMap map : types) {
      ...
      stats.add(StatType.wrapStatType(typeString, name, displayName, defaultValue, displayIndex,
          displayLineIndex,
  ...
  public StatType<?> getStat(String gameName, String name) {
    ...
    return StatType.wrapStatType(typeString, name, displayName, defaultValue, displayIndex,
        displayLineIndex,
        globalDisplay, globalDisplayIndex, globalDisplayLineIndex);
  }

  public void addStat(String gameName, StatType<?> stat) {
    super.addEntry(new PrimaryKeyEntries(
            new Entry<>(gameName, Column.Game.GAME_NAME),
            new Entry<>(stat.getName(), Column.Game.STAT_TYPE_NAME)),
        ...
        new Entry<>(stat.getGlobalDisplayLineIndex(),
            Column.Game.STAT_TYPE_GLOBAL_DISPLAY_LINE_INDEX));
  }

  public void removeStat(String gameName, StatType<?> stat) {
    super.deleteEntrySynchronized(new Entry<>(gameName, Column.Game.GAME_NAME),
        new Entry<>(stat.getName(), Column.Game.STAT_TYPE_NAME));
  }
